...
def get_featurevalue(dataset,featurename):
    featuretypepath = os.path.join(os.getcwd(),'db',dataset,'featuretypes.csv')
    types = pd.read_csv(featuretypepath)
    featurepath = os.path.join(os.getcwd(),'db',dataset,'features.csv')
    df = pd.read_csv(featurepath,nrows=rows)
...
@app.route("/api/<dataset>/featureinfo")
...
@app.route("/api/<dataset>/calFeatureProjection/<method>",methods=['POST'])
...
def get_shapproj(dataset,modelname):
    ...
    featurepath = os.path.join(os.getcwd(),'db',dataset,'features.csv')
    classdf = pd.read_csv(featurepath,nrows=rows)[classfeature]
    modelpath = os.path.join(os.getcwd(),'db',dataset,'models.json')
...
def get_predictions(dataset):
    predictionpath = os.path.join(os.getcwd(),'db',dataset,'predictions.csv')
    df = pd.read_csv(predictionpath,nrows=rows)
    result={}
    for column in df:
...
def calmodelsinfos(dataset,modelnames):
    predictionpath = os.path.join(os.getcwd(),'db',dataset,'predictions.csv')
    df = pd.read_csv(predictionpath,usecols=modelnames+['key','label'])
    predictions=[]
...
def get_modelstatistics_subset(dataset):
    ...
    predictionpath = os.path.join(os.getcwd(),'db',dataset,'predictions.csv')
    df = pd.read_csv(predictionpath,usecols=modelnames+['key','label']).iloc[subset]
    statistics = []

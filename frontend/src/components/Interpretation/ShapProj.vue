<template>
    <div id="shapprojectioncanvas" ref="resizeRef">
...
import { select, extent, scaleLinear, brush, scaleSequential, interpolateYlGnBu, scaleOrdinal, schemeSet2 } from 'd3'
  ...
  setup (props) {
    ...
    const renderPoints = (data, encodingfeature, width, height) => {
      ...
      if (encodingfeature.type === 'numerical') {
        ...
        mycolorscale = scaleOrdinal().domain(e).range(schemeSet2)
        legend({
          svg: svglengend,
          color: scaleOrdinal(e, schemeSet2),
          title: encodingfeature.name
        })
      }
